...
\begin{abstract}
We compare three state-of-the-art unconditional covariance estimators---Linear Shrinkage (LS), Quadratic Inverse Shrinkage (QIS), and Average Oracle (AO)---as priors for the Dynamic Conditional Correlation (DCC) model in large-scale portfolio optimization. Using daily returns from the 500 largest U.S. equities from 2000 to 2024, we devolatize returns via exponentially weighted moving average volatility estimates, then apply each estimator to standardized returns. The resulting unconditional covariance matrices serve as targets in the DCC recursion, producing time-varying conditional covariance matrices for global minimum-variance (GMV) portfolio construction.

Our empirical evaluation reveals that LS consistently delivers the strongest risk-adjusted performance in unconstrained settings, achieving an annualized Sharpe ratio of 1.58 with 81.5\% cumulative return and maximum drawdown of $-3.8$\%. QIS follows closely with a Sharpe ratio of 1.41 and 66.0\% cumulative return, while requiring slightly higher leverage and turnover. AO exhibits the lowest volatility (13 bps) but underperforms with a Sharpe ratio of 1.02 and 37.9\% cumulative return, largely due to aggressive rebalancing and higher leverage requirements. In long-only portfolios, the three methods converge to similar performance metrics, with cumulative returns clustering between 372\% and 391\%, indicating that covariance regularization primarily impacts unconstrained allocations.

These findings suggest that LS (and by extension QIS) provides the most robust foundation for DCC-based portfolio optimization, while AO may require longer estimation windows or additional shrinkage to compete effectively. Our study represents the first comprehensive comparison of these modern estimators within the DCC framework using real market data.
\end{abstract}
...
We address the problem of estimating conditional covariance matrices for large-scale equity portfolios by combining unconditional covariance estimators with the Dynamic Conditional Correlation (DCC) framework of \cite{engle2002}. Specifically, we compare three state-of-the-art unconditional covariance estimators---Linear Shrinkage (LS) \cite{ledoit2004}, Quadratic Inverse Shrinkage (QIS) \cite{ledoit2019}, and Average Oracle (AO) \cite{bongiorno2024}---as priors for the DCC model.
...
The key research question is: which unconditional covariance estimator, when embedded in the DCC framework, delivers superior risk-adjusted returns and portfolio characteristics in both unconstrained and long-only settings?
